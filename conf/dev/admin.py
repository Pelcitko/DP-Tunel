    ...
    def count(self):
        ...
        try:
            with connection.cursor() as cursor:
                cursor.execute('SET LOCAL statement_timeout TO 300000;')
                return super().count
...
class AllDataAdmin(ChartAdmin, DefaultFilterAdmin):
    ...
    list_display = ['id_sensor', 'time', 'data_as_link', 'calculate_flow', 'point_number', 'type_number',
                    'id_sensor_as_link', 'device_as_link', 'point_number', ]
    # list_select_related = ('id_sensor__id_device', ) # Non-relational field given in select_related: 'id_sensor'. Choices are: (none)
    # /nelze optimalizovat bez přpsání queryset
...
class DataAdmin(ChartAdmin, DefaultFilterAdmin):
    ...
    show_full_result_count = False
    # paginator = TimeLimitedPaginator
    default_filters = ('time__gte={}+00:00'.format(timezone.now().date()-timedelta(weeks=100)),)
    date_hierarchy = 'time'
    list_display = ['id_sensor', 'time', 'value', 'unit', 'calculate_flow', 'type_as_link', 'get_point_as_link',
                    'sensor_number', 'device_as_link', ]
    list_filter = [
